...
import { parseGitHubUrl, resolveCredential, fetchAllPages, normalizePullRequestData, normalizePrFile, normalizeReview, groupReviewCommentsIntoThreads, normalizeCheckRun, normalizeIssue, createTombstone, buildEnterpriseHostIdentity } from "@fuzit/provider-github";
...
import { getRemoteCacheInfo, createDisposableWorktree, checkOfflineCache, inferRemoteFromGitConfig } from "@fuzit/git";
...
describe("GH-031 Acceptance Suite: End-to-End GitHub Provider Verification", () => {
  ...
  it("3. Base/head PR acquisition & PR URL dispatch", async () => {
    ...
    if (!parsed.ok || parsed.ref.kind !== "github-pull-request") throw new Error("Expected PR ref");
    ...
    const reviewRes = await runPrReview({ prRef });
  ...
  it("4. Changed files and bounded patches", () => {
    ...
    if (!parsed.ok || parsed.ref.kind !== "github-pull-request") throw new Error("Expected PR ref");
    const prRef = parsed.ref;
    const { fileRecord, patchRecord } = normalizePrFile(prRef, {
  ...
  it("5. Reviews, review comments, issue comments, and threads", () => {
    ...
    if (!parsed.ok || parsed.ref.kind !== "github-pull-request") throw new Error("Expected PR ref");
    const prRef = parsed.ref;
    const review = normalizeReview(prRef, { id: 77, state: "APPROVED", user: { login: "bob" } });
    expect(review.reviewId).toBe(77);

    const threads = groupReviewCommentsIntoThreads(prRef, [
      { id: 1, path: "file.ts", line: 10, body: "Look at this", user: { login: "bob" } },
      { id: 2, path: "file.ts", line: 10, in_reply_to_id: 1, body: "Fixed", user: { login: "alice" } },
    ]);
  ...
  it("7. Issue context & Issue URL dispatch", async () => {
    ...
    if (!parsed.ok || parsed.ref.kind !== "github-issue") throw new Error("Expected issue ref");
    const issueRef = parsed.ref;
    const issueRecord = normalizeIssue(issueRef, { title: "Bug report", body: "App crashes", state: "open" });
    expect(issueRecord.number).toBe(10);

    const issueRes = await runIssueContext({ issueRef });
  ...
  it("10. Tombstones for deleted records and refs", () => {
    const host = { webHost: "github.com", apiHost: "api.github.com", isEnterprise: false };
    const tombstone = createTombstone(host, "owner/repo", "review-comment", 999, "deleted");
  ...
  it("12. Security: Deny-by-default network policy & redirect validation", () => {
    const localPolicy = evaluateNetworkPolicy({ kind: "local", path: "/workspace" }, "scan");
    expect(localPolicy.authorized).toBe(false);

    const redirectAllowed = isRedirectAllowed("https://evil.com/phish", ["github.com"]);
    expect(redirectAllowed).toBe(false);
  });

  it("13. Remote inference from local clone remotes", () => {
    const inferred = inferRemoteFromGitConfig([{ name: "origin", url: "https://github.com/org/repo.git" }]);
    expect(inferred).toEqual({ owner: "org", repo: "repo" });
  });

  it("14. Disposable worktree creation and cleanup", async () => {
    const cacheInfo = getRemoteCacheInfo("github.com", "owner", "repo");
    const { worktreePath, cleanup } = await createDisposableWorktree(cacheInfo, "head456");
